...
def test_agent_uses_query_api_for_item_count():
    ...
    assert "query_api" in tool_names, (
        "Expected 'query_api' in tool_calls for this question"
    )
    ...
    assert len(numbers) > 0, "Expected answer to contain a number"
...
def test_agent_uses_query_api_for_status_code():
    """Test that agent uses query_api tool to check API status codes."""
    # Path to agent.py in project root
    project_root = Path(__file__).parent.parent.parent.parent
    agent_path = project_root / "agent.py"
